...
const GroupLayoutPreview = ({
  ...
  const copyLayoutId = useCallback(async (layoutIndex: number) => {
    ...
    try {
      ...
      notify.success("已复制", "页面 ID 已复制到剪贴板。");
    } catch {
  ...
  const copyTemplateId = useCallback(async () => {
    try {
      ...
      notify.success("已复制", "模板 ID 已复制到剪贴板。");
    } catch (copyError) {
      notify.error(
        "复制失败",
