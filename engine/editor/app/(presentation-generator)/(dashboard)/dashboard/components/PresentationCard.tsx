...
export const PresentationCard = ({
  ...
  const handleDelete = async () => {
    ...
    if (response?.success) {
      ...
      notify.success("演示文稿已删除", "已从项目列表中移除。" );
      setShowDeleteDialog(false);
      if (onDeleted) {
        onDeleted(id);
      }
    } else {
      notify.error("无法删除演示文稿", response?.message || "删除演示文稿时发生错误。" );
    }
    setIsDeleting(false);
  };
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      notify.success("演示文稿已复制", "副本已加入项目列表。" );
      onDuplicated?.(duplicated);
    } catch (error) {
      notify.error(
        "无法复制演示文稿",
        error instanceof Error ? error.message : "复制演示文稿时发生错误。"
      );
    } finally {
        ...
        <div className={isUnsupported
          ...
          {isUnsupported ? (
            <div className="flex flex-col items-center gap-2 px-5 text-center text-[#666666]">
              ...
              <p className="text-xs font-medium">暂无预览</p>
            </div>
