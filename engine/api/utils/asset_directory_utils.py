...
def resolve_app_path_to_filesystem(path_or_url: str) -> Optional[str]:
    ...
    # TeachNova production exposes FastAPI assets under /ppt-api/app_data/.
    # Normalize the public reverse-proxy prefix back to the internal app path.
    if path.startswith("/ppt-api/app_data/"):
        path = "/app_data/" + path[len("/ppt-api/app_data/"):]
...
def _app_data_url_allowed(path: str) -> bool:
    ...
    # Template preview PPTX/images are generated by the server and later
    # consumed by the asynchronous template creation task. They are stored
    # under a shared transient directory rather than an owner-scoped folder.
    if len(parts) >= 2 and parts[0] == "uploads" and parts[1] == "template-previews":
        return True
