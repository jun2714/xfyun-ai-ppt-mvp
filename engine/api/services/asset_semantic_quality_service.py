...
def _build_quality_prompt(
    ...
    return (
        ...
        "检查物种真实结构、身体部件归属与动作是否符合教学描述。人兽融合、器官替换、"
        "用人物合影替代应观察的动物，或无关装饰压过教学主体，应判 features_match=false。"
        "幼儿模仿动物指正常儿童做动作，不是身体变成动物。"
        "背景类契约不要求 detected_count 精确。还必须检查整张图片：只要出现英文、"
        "字母、汉字、数字、标签、标志、水印或伪文字，就将 visible_text_detected 设为 true；"
        "要求完整展示的主体或关键动作被画面边缘截断时，将 subject_cropped 设为 true。"
        "契约明确要求局部猜谜、剪影或遮挡线索时，只检查所需线索是否完整可辨，"
        "不要把这种有意遮挡误判为裁切；背景边缘的次要装饰不算主体裁切。\n"
