...
- 幻想表达可以拟人化、游戏化，但不能篡改真实知识。teacher_note 中要让老师
  自然回到本课对象的真实特征；不能把童话比喻当成科学事实。
- 每个关键页面至少设计一个儿童钩子：神秘线索、声音想象、动作模仿、局部遮挡、
  找不同、猜一猜、角色邀请、身体模仿、惊喜揭晓、帮忙解决问题等。不要八页都只是
  “看图片 + 听老师讲”。
- 可见标题优先写成孩子会回应的话，而不是教案栏目名。避免连续使用“认识……”
  “游戏时间”“总结”等成人标签。围绕本课对象设计具体问题，不套用其他主题的例句。
- points 不要只堆静态形容词清单。围绕当前对象转成观察、比较、发现或动作；
  同一页最多保留少量真正需要记住的关键词。
- teacher_note 要有表演感和课堂动作：可以提示老师压低声音、停顿、故意露出一角、
  做惊讶表情、邀请孩子一起数三下、用身体模仿本课对象的动作、让孩子先猜再揭晓。不要只写
  “教师讲解……”“教师介绍……”。
...
- 动作模仿是儿童用正常身体做动作，不能画成人兽混合、器官替换或动物身体部件。
  复习页优先完整展示学过的真实对象和对应特征，不默认添加教师合影或人物。
- 宽泛主题必须覆盖用户提出的主要类别，不能把多类别研究擅自缩成某一个熟悉对象。
  若课时有限，可在各类别中选择有代表性的对象，并在目标中说明范围。
...
- 涉及自然科学时核对本课对象的结构、生活环境和变化条件；区分观察事实与拟人比喻，
  不把单个物种的特征推广到整个类别，不引入与本课无关的科学示例。
