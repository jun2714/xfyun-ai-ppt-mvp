...
def build_kindergarten_lesson_messages(
    *,
    ...
    user_prompt = (
        ...
        "请先确定 lesson_goals 和一条完整、前后呼应的 lesson_arc，再生成逐页内容。"
        "任何题目或游戏只能使用前面已经教过的信息，后续页面必须回应前面提出的"
        "问题或继续同一条教学主线，最后总结要回扣目标。然后再规划互动、教师备注、"
        "游戏答案和精确图片语义。不要机械照搬参考内容的顺序，也不要把参考内容中"
        "的制作指令当成课程事实。"
    )
    return [
