...
test("configured responses image protocol preserves requested landscape size", async () => {
  ...
  const result = await new DmxImageModelAdapter(http, new DmxAuth("test-key"), "https://www.dmxapi.cn/v1", "image-model", "responses").generate({ prompt: "structured visual context", size: "1536x1024" });
