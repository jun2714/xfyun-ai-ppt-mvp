...
test("qwen-image-2.0 uses DMX responses API and extracts image output",async()=>{
 let call:{url:string;body:unknown}|undefined;
 const http={post:async(url:string,_headers:Record<string,string>,body:unknown)=>{call={url,body};return{output:[{type:"message",content:[{type:"image",text:"https://example.com/generated.png"}]}]}}} as unknown as JsonHttpClient;
