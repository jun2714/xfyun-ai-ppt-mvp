...
const Id=z.object({id:z.string().min(1)}),PageId=z.object({id:z.string().min(1),pageId:z.string().min(1)}),Expected=z.object({expectedRevision:z.number().int().nonnegative(),idempotencyKey:z.string().min(1)});
...
const Command=Expected.extend({type:z.enum(["set-text","set-bounds","set-z","set-asset","set-crop"]),pageId:z.string(),nodeId:z.string(),value:z.union([z.string(),z.number(),z.object({x:z.number(),y:z.number(),width:z.number().positive(),height:z.number().positive()})])});
...
export function buildApp() {
  ...
  app.get("/api/v1/health",async()=>({status:"ok",generationEnabled:container.config.dmxApiKey.length>0,schemaVersion:"007.1"}));
  ...
  app.get("/api/v1/presentations/:id",async request=>success(container.platform.get(Id.parse(request.params).id).brief,request.id));
  app.post("/api/v1/presentations/:id/outline-jobs",async(request,reply)=>reply.status(202).send(success(container.platform.startOutline(Id.parse(request.params).id),request.id)));
  ...
  app.post("/api/v1/presentations/:id/design-jobs",async(request,reply)=>reply.status(202).send(success(container.platform.startDesign(Id.parse(request.params).id),request.id)));
  ...
  app.post("/api/v1/presentations/:id/composition-jobs",async(request,reply)=>{const id=Id.parse(request.params).id,body=z.object({canvas:z.object({width:z.number().positive(),height:z.number().positive()}).optional(),idempotencyKey:z.string()}).parse(request.body);return reply.status(202).send(success(container.platform.compose(id,body.canvas),request.id))});
  ...
  app.post("/api/v1/presentations/:id/asset-jobs",async(request,reply)=>reply.status(202).send(success(await container.platform.resolveAssets(Id.parse(request.params).id),request.id)));
  app.post("/api/v1/presentations/:id/quality-jobs",async(request,reply)=>reply.status(202).send(success(container.platform.quality(Id.parse(request.params).id),request.id)));
  app.post("/api/v1/presentations/:id/repair-jobs",async(request,reply)=>reply.status(202).send(success(container.platform.repair(Id.parse(request.params).id),request.id)));
