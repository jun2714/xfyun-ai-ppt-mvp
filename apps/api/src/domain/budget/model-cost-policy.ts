...
export class ModelCostPolicy {
  ...
  estimateText(inputTokens: number, outputTokens: number): number {
    return this.round(
      inputTokens * this.config.textInputRmbPerMillion / 1_000_000
      + outputTokens * this.config.textOutputRmbPerMillion / 1_000_000
    );
  }

  estimateImage(): number {
    return this.round(this.config.imageRmbPerCall);
  }

  assertWithinBudget(estimatedRmb: number): void {
    if (estimatedRmb > this.config.maxRequestRmb) {
      throw new AppError(
        "BUDGET_EXCEEDED",
        `Estimated model cost ¥${estimatedRmb} exceeds request limit ¥${this.config.maxRequestRmb}`,
        402
      );
    }
  }
