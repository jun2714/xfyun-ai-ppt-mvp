...
const chartSeries = (rows: unknown) => {
  ...
  return header.slice(1).map((name, column) => ({
    name: String(name ?? `Series ${column + 1}`),
    labels: categories,
  ...
  for (const page of scene.pages) {
    ...
    for (const node of [...page.nodes].sort((left, right) => left.zIndex - right.zIndex)) {
      ...
      if (node.kind === "text") {
        ...
        if (data) slide.addImage({ data, x: box.x, y: box.y, sizing: { type: node.content.fit === "contain" ? "contain" : "cover", w: box.w, h: box.h }, rounding: Number(node.style.radius ?? 0) > 0 });
        ...
        if (series.length) slide.addChart(pptx.ChartType.bar, series, {
          ...
          showTitle: false,
          showValue: true,
          chartColors: [color(scene.theme.primary, "4F9C67"), color(scene.theme.secondary, "F28C45"), color(scene.theme.accent, "F2C94C")],
          showCatName: false,
          showSerName: false,
          showPercent: false
        });
      } else if (node.kind === "connector") {
        slide.addShape(pptx.ShapeType.line, { ...box, line: { color: color(node.style.stroke, "333333"), width: Number(node.style.strokeWidth ?? 1.5), beginArrowType: "none", endArrowType: "triangle" } });
      }
    }
  }
