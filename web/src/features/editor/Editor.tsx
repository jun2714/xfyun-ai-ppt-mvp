...
export function Editor({ presentationId }: { presentationId: string }) {
  ...
  return <main className="editor-page"><header><a className="brand" href="/"><img className="brand-logo" src="/teachnova-logo.png" alt="Teachnova" /><span>幼教PPT</span></a><div><b>{presentation.title || "未命名演示"}</b><span>{presentation.slides.length} 页 · 自动保存</span></div><a href={`/presentations/${presentationId}/outline`}>查看大纲</a></header>
